...
int shot_analyze( unsigned short position_global, eFieldInfo* field, eFieldInfo* shot_field ) {
	...
	int ship_size;
	// checking was that place already shot
	if (shot_field[position_y * FIELD_SIZE + position_x] == HIT
	    || shot_field[position_y * FIELD_SIZE + position_x] == SHOT
	    || shot_field[position_y * FIELD_SIZE + position_x] == KILL) {
...
int kill_check( unsigned short position_global, eFieldInfo* field, eFieldInfo* shot_field ) {
	...
	while (shot_field[y_up * FIELD_SIZE + x] == HIT) {
		// up
	...
	while (shot_field[y_down * FIELD_SIZE + x] == HIT) {
		// down
	...
	while (shot_field[y * FIELD_SIZE + x_left] == HIT) {
		// left
	...
	while (shot_field[y * FIELD_SIZE + x_right] == HIT) {
		// right
