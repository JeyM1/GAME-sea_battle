...
int main(void){
    ...
    while(!gameover){           //main game thread
        switch(game_state){
            ...
            case DRAW:          //drawing and entering data
                {
                    ...
                    if(player == PLAYER_1 && net_player == SERVER){
                        ...
                        printf("Does not received anything\n");
                        break;
                    }
                }
                    ...
                    if((shot_pos >> 8) == FIELD_SIZE){
                        if(shot_pos%2 == 0)
                            if(choice_yes_no("Back to menu?\n")){
                                ...
                                if(send((net_player == SERVER ? newSHandle : sHandleClient), (char*)&status, sizeof(status), MSG_DONTROUTE) == SOCKET_ERROR){
                                    getSocketError();
                                    closesocket(sHandleClient);
                                    closesocket(sHandleServer);
                                    closesocket(newSHandle);
